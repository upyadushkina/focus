...
function reversedList() {
  ...
  nodeGroups.each(function(d) {
    // Check if node has "done" in fields, order_tag, or description
    const hasDone = (d.fields && d.fields.some(field => field.toLowerCase().includes('done'))) ||
                    (d.orderTag && d.orderTag.toLowerCase().includes('done')) ||
                    (d.description && d.description.toLowerCase().includes('done'));
    if (hasDone) {
